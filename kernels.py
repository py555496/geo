...
def local_cdist(coords_i, coords, spherical):
    ...
    if spherical:
        ...
        return np.sqrt(np.sum((np.array(coords_i) - np.array(coords))**2, axis=1))
...
class Kernel(object):
    ...
    def __init__(self, i, data, bw=None, fixed=True, function='triangular',
        ...
        self.kernel = np.array(self._kernel_funcs(self.dvec / self.bandwidth),dtype=np.float32)
        #print("dvec",self.dvec/self.bandwidth)
